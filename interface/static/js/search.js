function SearchResults() {
	...
	function FlightResult({
		...
		return (
			<div
				...
				onClick={() => onClick([result], flightData, false)}
			>
	...
	function ConnectingFlightResult({
		...
		return (
			<div
				...
				onClick={() => onClick(resultPair, flightData, true)}
			>
				<h2>Flight 1</h2>
				<FlightResult
					result={resultPair[0]}
	...
	function selectToFlight(result, flightData, isConnecting) {
		if (isConnecting) {
			const { 0: firstFlight, 1: secondFlight } = result;
			result = [firstFlight, secondFlight];
		}

		const selectedFlight = result.map((res) => ({
			id: res[0],
	...
	function selectReturnFlight(result, flightData, isConnecting) {
		if (isConnecting) {
			const { 0: firstFlight, 1: secondFlight } = result;
			result = [firstFlight, secondFlight];
		}
